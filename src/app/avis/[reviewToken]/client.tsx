...
export function AvisInviteClient({ reviewToken, data, error }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/avis/invitation/${reviewToken}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ note, commentaire: commentaire.trim() || undefined }),
      });
          ...
          <CardContent className="space-y-4">
            {error ? (
              ...
              <>
                ...
                {data.auteurNom && (
                  <p className="text-sm text-muted-foreground text-center">
                    Bonjour <span className="font-medium text-foreground">{data.auteurNom}</span>,
