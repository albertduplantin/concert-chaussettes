...
import { DemandesContactList } from "./demandes-contact-list";
import { Mic2, Sparkles } from "lucide-react";

export default async function OrganisateurDemandesPage() {
  ...
  const demandes = await db.query.demandesContact.findMany({
    where: eq(demandesContact.organisateurId, organisateur.id),
    orderBy: [desc(demandesContact.createdAt)],
    with: {
      groupe: { columns: { id: true, nom: true, thumbnailUrl: true, contactEmail: true, contactTel: true } },
    },
  });

  const unreadCount = demandes.filter((d) => !d.isRead).length;
    ...
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-orange-500 to-amber-500 p-8 text-white">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold mb-2">Demandes de concert</h1>
          <p className="text-white/90 max-w-xl">
            Les groupes intéressés pour jouer chez vous vous ont contacté ici.
            Répondez-leur directement par email.
          </p>
        </div>
      </div>

      <DemandesContactList demandes={demandes} />
    </div>
  );
}
